...
class SecurityAuthorizationTest {
  ...
  @MockitoBean private OneTimeTokenGenerationSuccessHandler oneTimeTokenGenerationSuccessHandler;
  ...
  void getCheckEmailIsPublic() throws Exception {
    mockMvc.perform(get("/check-email")).andExpect(status().isOk());
  }

  @Test
  void getMoviesWithoutAuthRedirectsToLogin() throws Exception {
    mockMvc.perform(get("/movies")).andExpect(status().is3xxRedirection());
  }
  ...
  void getMoviesWithUserRole() throws Exception {
    mockMvc
        .perform(get("/movies").with(user("user").authorities(new SimpleGrantedAuthority("USER"))))
        .andExpect(status().isOk());
  }
  ...
  void getMoviesWithAdminRole() throws Exception {
    mockMvc
        .perform(
            get("/movies").with(user("admin").authorities(new SimpleGrantedAuthority("ADMIN"))))
        .andExpect(status().isOk());
  }
  ...
  void getUsersWithUserRoleForbidden() throws Exception {
    mockMvc
        .perform(get("/users").with(user("user").authorities(new SimpleGrantedAuthority("USER"))))
  ...
  void getUsersWithAdminRole() throws Exception {
    mockMvc
        .perform(get("/users").with(user("admin").authorities(new SimpleGrantedAuthority("ADMIN"))))
        .andExpect(status().isOk());
  }
}
